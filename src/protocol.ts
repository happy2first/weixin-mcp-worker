...
export async function uploadMediaBuffer(env: Env, params: {
  ...
  const mediaType = mediaTypeNumber(params.kind);
  ...
  const fail = (stage: string, message: string, extra: Record<string, unknown> = {}) => {
    const diagnostic = { stage, ...baseDiagnostic, elapsedMs: Date.now() - startedAt, ...extra };
    console.error("Weixin media upload diagnostic:", JSON.stringify(diagnostic));
    return new Error(`${message}；diagnostic=${JSON.stringify(diagnostic)}`);
  };

  let upload: GetUploadUrlResponse;
  try {
    upload = await getUploadUrl(env, {
    ...
    throw fail("getuploadurl", `微信 getUploadUrl 失败：${detail}`, { errorName: error instanceof Error ? error.name : typeof error });
  }

  const uploadSource = upload.upload_full_url?.trim() ? "upload_full_url" : upload.upload_param ? "upload_param" : "missing";
  ...
  if (!rawTarget) throw fail("getuploadurl_response", "微信 getUploadUrl 未返回上传地址", { uploadSource });

  let target: string;
  let uploadHost = "unknown";
  try {
    target = normalizeHttpsUrl(rawTarget);
    uploadHost = new URL(target).host;
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error);
    throw fail("getuploadurl_response", `微信 getUploadUrl 返回了无效上传地址：${detail}`, { uploadSource });
  }
  ...
  try {
    ...
    const detail = error instanceof Error ? error.message : String(error);
    throw fail("cdn_upload_request", `微信 CDN 上传请求失败：${detail}`, {
      uploadHost,
      uploadSource,
  ...
  const responseHeaders = {
    ...
    xErrorMessage: response.headers.get("x-error-message"),
    xRequestId: response.headers.get("x-request-id"),
    xCosRequestId: response.headers.get("x-cos-request-id"),
    xNwsLogUuid: response.headers.get("x-nws-log-uuid"),
  };

  if (!response.ok) {
    const bodyText = await response.text().catch(() => "");
    const bodySummary = bodyText.replace(/\s+/g, " ").trim().slice(0, 1000);
    throw fail("cdn_upload_response", `微信 CDN 上传失败：HTTP ${response.status}${response.statusText ? ` ${response.statusText}` : ""}`, {
      uploadHost,
      uploadSource,
      requestElapsedMs: Date.now() - requestStartedAt,
  ...
  if (!downloadEncryptedQueryParam) {
    throw fail("cdn_upload_response_validation", "微信 CDN 上传成功但缺少 x-encrypted-param", {
      uploadHost,
      uploadSource,
      requestElapsedMs: Date.now() - requestStartedAt,
      httpStatus: response.status,
