...
import { MAX_MEDIA_BYTES, MCP_EMBED_MAX_BYTES } from "./media-v051.js";
import type { Env, SendableMediaKind, WeixinUserProfile } from "./types.js";
export { WeixinBotDO } from "./bot.js";
...
const callRegistry = (env: Env, path: string, init?: RequestInit) => callStub(stubByName(env, REGISTRY_DO), path, init);
const userStub = (env: Env, userId: string) => stubByName(env, `user:${normalizeProfileId(userId)}`);
const callUser = (env: Env, userId: string, path: string, init?: RequestInit) => callStub(userStub(env, userId), path, init);
...
async function usersWithStatus(env: Env) {
  ...
  const users = await Promise.all(list.map(async (user) => {
    ...
    catch (error) { return { ...user, status: { connected: false, historyBytes: 0, historyLimitBytes: DEFAULT_RETENTION_LIMIT_BYTES, error: error instanceof Error ? error.message : String(error) } }; }
  }));
  const totalHistoryBytes = users.reduce((sum, user) => sum + Number((user.status as JsonObject).historyBytes || 0), 0);
  const totalHistoryLimitBytes = users.reduce((sum, user) => sum + Number((user.status as JsonObject).historyLimitBytes || DEFAULT_RETENTION_LIMIT_BYTES), 0);
  return {
    users,
    storagePlan: {
      totalHistoryBytes,
      totalHistoryLimitBytes,
      safeBudgetBytes: SAFE_PROJECT_HISTORY_BUDGET_BYTES,
      withinSafeBudget: totalHistoryLimitBytes <= SAFE_PROJECT_HISTORY_BUDGET_BYTES,
    },
  };
}
...
async function sendToRecipients(env: Env, text: string, requested?: string[]) {
  ...
  const results = await Promise.all(targets.map(async (user) => {
    try {
      const response = await callUser(env, user.id, "/send", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ text }) });
...
function routeMediaMetadata(metadata: unknown, user: WeixinUserProfile): unknown {
  ...
  if (Array.isArray(copy.media)) copy.media = copy.media.map((media: JsonObject) => ({ ...media, mediaRef: media.mediaRef ? `${user.id}:${media.mediaRef}` : media.mediaRef, user: { id: user.id, name: user.name } }));
  return copy;
}
...
async function readGlobalMedia(env: Env, globalMediaRef: string) {
...
async function resolveMediaSource(env: Env, params: { dataBase64?: string; sourceMediaRef?: string; kind: SendableMediaKind; mimeType?: string; fileName?: string }) {
  ...
  if (hasBase64) {
    ...
    if (bytes.byteLength > MAX_MEDIA_BYTES) throw new Error(`媒体超过 ${Math.floor(MAX_MEDIA_BYTES / MIB)}MB 上限`);
    return { bytes, kind: params.kind, mimeType: params.mimeType?.trim() || "application/octet-stream", fileName: params.fileName?.trim() || (params.kind === "image" ? "image.jpg" : params.kind === "video" ? "video.mp4" : "file.bin") };
  }
  ...
  return {
    ...
    fileName: decodeURIComponent((media.response.headers.get("content-disposition") || "").match(/filename\*=UTF-8''([^;]+)/i)?.[1] || "") || params.fileName?.trim() || (sourceKind === "image" ? "image.jpg" : sourceKind === "video" ? "video.mp4" : "file.bin"),
  };
}

async function sendMediaToRecipients(env: Env, params: { recipients?: string[]; kind: SendableMediaKind; dataBase64?: string; sourceMediaRef?: string; mimeType?: string; fileName?: string; caption?: string }) {
...
async function mediaToolResult(env: Env, globalMediaRef: string) {
  ...
  if (size > MCP_EMBED_MAX_BYTES) return result({ mediaRef: globalMediaRef, kind, mimeType, fileName, sizeBytes: size, embedded: false, reason: `媒体超过 MCP 内嵌安全上限 ${Math.floor(MCP_EMBED_MAX_BYTES / MIB)}MB，可在 /admin 下载查看。` });
  ...
  if (mimeType.startsWith("image/")) return { content: [meta, { type: "image", data, mimeType }] as any };
  if (["audio/mpeg", "audio/wav", "audio/ogg", "audio/mp4"].includes(mimeType)) return { content: [meta, { type: "audio", data, mimeType }] as any };
  return { content: [meta, { type: "resource", resource: { uri: `weixin-media://${encodeURIComponent(globalMediaRef)}`, mimeType, blob: data } }] as any };
}

function createServer(env: Env) {
  const server = new McpServer({ name: "weixin-mcp-worker", version: VERSION });
  server.registerTool("weixin_users", { description: "列出 /admin 中维护的微信用户、默认用户、绑定状态和存储计划。", inputSchema: {} }, async () => result(await usersWithStatus(env)));
...
async function handleAdmin(request: Request, env: Env, pathname: string): Promise<Response> {
  ...
  if (request.method === "GET" && pathname === "/admin/api/messages") {
    ...
    return Response.json(await aggregateMessages(env, limit, offset));
  }
  if (request.method !== "POST") return Response.json({ error: "method_not_allowed" }, { status: 405 });
  const body = await request.json().catch(() => ({})) as Record<string, unknown>;
  if (pathname === "/admin/api/users/create") {
    const snapshot = await usersWithStatus(env);
    if (snapshot.storagePlan.totalHistoryLimitBytes + DEFAULT_RETENTION_LIMIT_BYTES > SAFE_PROJECT_HISTORY_BUDGET_BYTES) throw new Error("新增用户会使本项目历史保留配置超过 4GB 安全预算；请先降低现有用户额度");
  ...
  if (pathname === "/admin/api/users/update") return Response.json(await callRegistry(env, "/registry/update", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) }));
  ...
  if (pathname === "/admin/api/retention") {
    ...
    if (planned > SAFE_PROJECT_HISTORY_BUDGET_BYTES) throw new Error("该设置会使本项目历史保留上限合计超过 4GB 安全预算");
  ...
  if (pathname === "/admin/api/login/status") {
    const userId = normalizeProfileId(body.userId);
    return Response.json(await callUser(env, userId, "/login/status", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ sessionId: body.sessionId, verifyCode: body.verifyCode }) }));
  }
  ...
  if (pathname === "/admin/api/reply") {
    const userId = normalizeProfileId(body.userId);
    return Response.json(await callUser(env, userId, "/reply", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ messageRef: body.messageRef, text: body.text }) }));
  }
...
async function handleAdminMedia(env: Env, pathname: string) {
