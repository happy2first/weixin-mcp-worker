...
const BOT_AGENT = "weixin-mcp-worker/0.4.0";
...
export async function getUpdates(
  env: Env,
  params: { baseUrl: string; token: string; getUpdatesBuf?: string },
...
export async function sendTextMessage(
  env: Env,
  params: { baseUrl: string; token: string; toUserId: string; text: string; contextToken?: string },
): Promise<string> {
  return sendItem(env, {
    ...params,
...
export async function uploadMediaBuffer(
  ...
  const target = upload.upload_full_url?.trim()
  ...
  if (!target) throw new Error("微信 getUploadUrl 未返回上传地址");
...
export async function sendUploadedMediaMessage(
  env: Env,
  params: {
    baseUrl: string;
    token: string;
  ...
  if (params.kind === "image") {
    item = { type: 2, image_item: { media, mid_size: params.uploaded.fileSizeCiphertext } };
  } else if (params.kind === "video") {
    item = { type: 5, video_item: { media, video_size: params.uploaded.fileSizeCiphertext } };
  } else {
    item = {
      type: 4,
      file_item: { media, file_name: params.fileName || "file.bin", len: String(params.uploaded.fileSize) },
    };
  }
  return sendItem(env, {
    baseUrl: params.baseUrl,
